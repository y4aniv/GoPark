...
export default function CreateParkingDrawer({
  ...
  const form = useForm({
    ...
    validate: {
      name: (value) =>
        value.length < 3
          ? "Le nom du parking doit contenir au moins 3 caractères"
          : null,
      address: (value) =>
      ...
      zipCode: (value) =>
        value.length === 0
          ...
          : value.length !== 5 || isNaN(Number(value))
      ...
      levels: (value) =>
        value < 1
          ...
          : value % 1 !== 0
          ? "Le nombre de niveaux est invalide"
          : null,
      spotsPerLevel: (value) =>
        value < 1
          ? "Le nombre de places par niveau doit être supérieur à 0"
          : value % 1 !== 0
          ? "Le nombre de places par niveau est invalide"
          : null,
    },
  });
        ...
        onSubmit={form.onSubmit(async () => {
          ...
          try {
            const response = await axios.post(
              `${process.env.NEXT_PUBLIC_API_URL}/parkings/create`,
              {
            ...
            setParkings(
              [
                ...parkings,
                {
                  ...
                  available_spots:
                    response.data.parking.levels *
                    response.data.parking.spots_per_level,
                },
            ...
            switch (error.response.data.message) {
              case "PARKING_ALREADY_EXISTS":
                form.setFieldError(
                  "name",
                  `Le parking "${form.values.name}" existe déjà`
                );
                break;
              default:
                notifications.show({
                  title: "Impossible de créer le parking",
                  message: `Une erreur est survenue lors de la création du parking: ${
      ...
      >
        <Stack>
          ...
          <Group grow>
            ...
            />
            <NumberInput
              label="Nombre de places par niveau"
              placeholder="10"
              min={1}
              key={form.key("spotsPerLevel")}
              {...form.getInputProps("spotsPerLevel")}
            />
          </Group>
          <TextInput
            ...
            disabled
            value={
              isNaN(form.values.levels) ||
              form.values.levels % 1 !== 0 ||
              form.values.levels < 1 ||
              isNaN(form.values.spotsPerLevel) ||
              form.values.spotsPerLevel % 1 !== 0 ||
              form.values.spotsPerLevel < 1
                ? "N/A"
                : form.values.levels * form.values.spotsPerLevel
            }
          />
