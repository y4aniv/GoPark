  ...
  const form = useForm({
    ...
    validate: {
      licensePlate: (value) =>
        value.trim().length < 1
          ? "Ce champ est requis"
          : !/^[A-Z]{2}[0-9]{3}[A-Z]{2}$/.test(value)
          ? "Le numéro de plaque d'immatriculation doit être de la forme AA123AA"
          : null,
      brand: (value) =>
        value.trim().length < 1 ? "Ce champ est requis" : null,
      model: (value) =>
        value.trim().length < 1 ? "Ce champ est requis" : null,
      color: (value) =>
        value.trim().length < 1 ? "Ce champ est requis" : null,
        ...
        onSubmit={form.onSubmit(async () => {
          ...
          try {
            const response = await axios.post(
              `${process.env.NEXT_PUBLIC_API_URL}/cars/create`,
              {
            ...
            setCars(
              [...cars, response.data.car].sort((a: any, b: any) =>
            ...
            switch (error.response.data.message) {
              case "CAR_ALREADY_EXISTS":
                form.setFieldError(
                  "licensePlate",
                  `La voiture avec la plaque d'immatriculation "${form.values.licensePlate}" existe déjà`
                );
                break;
