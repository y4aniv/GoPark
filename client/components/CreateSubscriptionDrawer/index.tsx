import { Button, Drawer, Group, NumberInput, Select, Stack, TextInput, Title } from "@mantine/core";
...
export default function CreateSubscriptionDrawer({
    ...
    useEffect(()=>{
        if (parking){
          axios.get(`${process.env.NEXT_PUBLIC_API_URL}/parkings/${parking?.id}/spots/available`)
            .then((response) => {
        ...
        <form onSubmit={form.onSubmit(async() => {
          try {
            ...
            notifications.show({
              ...
              message: `Une erreur est survenue lors de la création de l'abonnement: ${error?.response?.data?.message ?? "UNKNOWN_ERROR"}`,
            })
          } finally {
            setLoading(false);
          }
          ...
          <Stack>
            ...
            />
            <Button type="submit" loading={loading}>Créer l'abonnement</Button>
          </Stack>
        </form>
      </Drawer>
    )
